...
const number = 3
const isEven = 3 % 2 ? false : true
console.log(`Число ${number} ${isEven ? 'чётное' : 'нечётное'}`)
...
function CDEK () {
  ...
  const type = prompt("Введите тип доставки: ", '')
  ...
  let basePrice;
  if (Number(weight) < 1) basePrice = 5
  if (Number(weight) >= 1 && Number(weight) <= 5) basePrice = 10
  if (Number(weight) > 5) basePrice = 15

  let coefficient
  if (type === 'Стандарт') coefficient = 1
  if (type === 'Экспресс') coefficient = 1.5
  if (type === 'Премиум') coefficient = 2
